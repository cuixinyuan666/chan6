...
4. Rust 后端是分型、笔、线段、N段、中枢、买卖点、节奏线、step 状态、BSP 特征、ML 打分的唯一计算权威。
...
15. step 模式必须作为后期逐 K 回放、回测、样本生成、BSP 特征提取和 ML 打分的基础。
16. BSP 特征必须与未来标签物理隔离，ML 打分只能依赖当时可见的特征，禁止未来函数。
17. 后续必须补充 ChanStepFrame / ChanBspPoint / ChanBspFeatureRow / ChanBspLabel / ChanBspMlScore。
"#;
