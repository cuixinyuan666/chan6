...
#[test]
fn chanpy_stage1_segment_extend_gold_matches_main_pipeline() {
    let csv =
        include_str!("../../../fixtures/chanpy_stage1/input/stage1_segment_extend_candidate.csv");
    let gold_text = include_str!(
        "../../../fixtures/chanpy_stage1/gold/stage1_segment_extend_candidate_chanpy_gold.json"
    );
    ...
    let actual = &snapshot.segments[0];
    let expected = &gold.seg[0];
    ...
    assert_eq!(actual.start_parent_index, Some(0));
    assert_eq!(actual.end_parent_index, Some(4));
    assert_eq!(actual.start_bar_id, 1);
    assert_eq!(actual.end_bar_id, 21);
    assert_eq!(actual.direction, ChanDirection::Up);
...
fn parse_stage1_csv(csv_text: &str) -> Vec<KLine1m> {
    csv_text
        ...
        .filter_map(|(index, line)| {
            ...
            Some(KLine1m {
                symbol: "stage1_segment_extend_candidate".to_string(),
                bar_id: index as i64,
                trading_day: 20260801 + index as i32,
                minute: 0,
