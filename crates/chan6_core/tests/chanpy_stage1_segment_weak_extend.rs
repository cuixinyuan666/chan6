...
#[test]
fn chanpy_stage1_segment_weak_extend_gold_matches_main_pipeline() {
    let csv = include_str!(
        "../../../fixtures/chanpy_stage1/input/stage1_segment_weak_extend_candidate.csv"
    );
    let gold_text = include_str!(
        "../../../fixtures/chanpy_stage1/gold/stage1_segment_weak_extend_candidate_chanpy_gold.json"
    );
    ...
    assert_eq!(snapshot.segments[1].end_price, 5.8);
...
fn parse_stage1_csv(csv_text: &str) -> Vec<KLine1m> {
    csv_text
        ...
        .filter_map(|(index, line)| {
            ...
            Some(KLine1m {
                symbol: "stage1_segment_weak_extend_candidate".to_string(),
                bar_id: index as i64,
                trading_day: 20261210 + index as i32,
                minute: 0,
