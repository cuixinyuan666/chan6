...
#[test]
fn chanpy_stage1_segment_third_reversal_gold_matches_main_pipeline() {
    let csv = include_str!(
        "../../../fixtures/chanpy_stage1/input/stage1_segment_third_reversal_candidate.csv"
    );
    let gold_text = include_str!(
        "../../../fixtures/chanpy_stage1/gold/stage1_segment_third_reversal_candidate_chanpy_gold.json"
    );
    ...
    assert_eq!(snapshot.segments[2].start_parent_index, Some(10));
    assert_eq!(snapshot.segments[2].end_parent_index, Some(12));
    assert_eq!(snapshot.segments[2].start_bar_id, 41);
    assert_eq!(snapshot.segments[2].end_bar_id, 53);
    assert_eq!(snapshot.segments[2].start_price, 2.5);
    assert_eq!(snapshot.segments[2].end_price, 16.5);
    assert_eq!(snapshot.segments[2].direction, ChanDirection::Up);
    assert!(!snapshot.segments[2].confirmed);
}
...
fn parse_stage1_csv(csv_text: &str) -> Vec<KLine1m> {
    csv_text
        ...
        .filter_map(|(index, line)| {
            ...
            Some(KLine1m {
                symbol: "stage1_segment_third_reversal_candidate".to_string(),
                bar_id: index as i64,
