...
use chan6_core::chan::model::{ChanDirection, ChanFxKind, CHAN_SEGMENT_N_LINE};
use chan6_core::model::KLine1m;
use serde::Deserialize;

#[test]
fn chanpy_stage1_bi_three_link_gold_matches_main_pipeline() {
    let csv =
        include_str!("../../../fixtures/chanpy_stage1/input/stage1_bi_three_link_candidate.csv");
    let gold_text = include_str!(
        "../../../fixtures/chanpy_stage1/gold/stage1_bi_three_link_candidate_chanpy_gold.json"
    );
    ...
    for (index, (actual, expected)) in snapshot.bi.iter().zip(&gold.bi).enumerate() {
        ...
        let expected_prev = index.checked_sub(1);
        let expected_next = if index + 1 < gold.bi.len() {
            Some(index + 1)
        } else {
            None
        };
        assert_eq!(actual.prev_index, expected_prev);
        assert_eq!(actual.next_index, expected_next);
    ...
    assert_eq!(snapshot.bi[0].direction, ChanDirection::Up);
    assert_eq!(snapshot.bi[1].direction, ChanDirection::Down);
    assert_eq!(snapshot.bi[2].direction, ChanDirection::Up);

    assert_eq!(snapshot.bi[0].next_index, Some(1));
    assert_eq!(snapshot.bi[1].prev_index, Some(0));
    assert_eq!(snapshot.bi[1].next_index, Some(2));
    assert_eq!(snapshot.bi[2].prev_index, Some(1));
    assert_eq!(snapshot.bi[2].next_index, None);
}
