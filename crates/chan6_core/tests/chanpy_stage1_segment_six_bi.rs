...
#[test]
fn chanpy_stage1_segment_six_bi_gold_matches_main_pipeline() {
    let csv =
        include_str!("../../../fixtures/chanpy_stage1/input/stage1_segment_six_bi_candidate.csv");
    let gold_text = include_str!(
        "../../../fixtures/chanpy_stage1/gold/stage1_segment_six_bi_candidate_chanpy_gold.json"
    );
    ...
    assert_eq!(snapshot.segments[0].end_parent_index, Some(4));
    assert_eq!(snapshot.segments[0].start_bar_id, 1);
    assert_eq!(snapshot.segments[0].end_bar_id, 21);
    assert_eq!(snapshot.segments[0].direction, ChanDirection::Up);

    assert_eq!(snapshot.segments[1].start_parent_index, Some(5));
    assert_eq!(snapshot.segments[1].end_parent_index, Some(5));
    assert_eq!(snapshot.segments[1].start_bar_id, 21);
    assert_eq!(snapshot.segments[1].end_bar_id, 25);
...
fn parse_stage1_csv(csv_text: &str) -> Vec<KLine1m> {
    csv_text
        ...
        .filter_map(|(index, line)| {
            ...
            Some(KLine1m {
                symbol: "stage1_segment_six_bi_candidate".to_string(),
                bar_id: index as i64,
                trading_day: 20261001 + index as i32,
                minute: 0,
